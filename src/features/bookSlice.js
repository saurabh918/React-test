...
const initialState = {
  ...
  categories: ["Fiction","Non-Fiction","Mystery","Science Fiction","Biography"], // initially set categories
  authors: ["John Doe","Jane Smith","Mark Johnson","Emily Davis","Michael Wilson"] // initially set authors
}
...
export const bookSlice = createSlice({
  ...
  initialState,
  reducers: {
    setPage: (state,action) => {  // current page number 
      state.currentPage = action.payload
    },
    addBook: (state,action) => { // add new book
      state.bookData.push(action.payload)
    },
    addCategory: (state,action) => { // add new category
      const category = action.payload
      if(!(state.categories.includes(category))) {
        state.categories.push(category)
      }
    },
    addAuthor: (state,action) => { // add new author
      const author = action.payload
      if(!(state.authors.includes(author))) {
       state.authors.push(author) 
      }
    },
    editBookInfo: (state,action) => { // edit book
